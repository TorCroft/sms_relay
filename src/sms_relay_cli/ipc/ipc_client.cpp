...
bool IpcClient::connect() {
    ...
    if (ret != 0) {
        ...
        wsa_initialized_ = false;
#else
        std::cerr << "[IPC Client] getaddrinfo failed: " << gai_strerror(ret) << std::endl;
#endif
        return false;
    }
    ...
    if (!connected) {
        std::cerr << "[IPC Client] Failed to connect to " << host_ << ":" << port_ << std::endl;
#ifdef _WIN32
        if (wsa_initialized_) {
            WSACleanup();
            wsa_initialized_ = false;
    ...
    std::cout << "[IPC Client] Connected to " << host_ << ":" << port_ << std::endl;
...
std::vector<uint8_t> IpcClient::send_command(const std::vector<uint8_t>& request) {
    if (!connected_) {
...
#ifdef _WIN32
        int received = recv(sock_, reinterpret_cast<char*>(header_buf + header_received),
                           static_cast<int>(16 - header_received), 0);
#else
        ssize_t received = recv(sock_, header_buf + header_received, 16 - header_received, 0);
#endif
        if (received <= 0) {
    ...
    if (!IpcSerializer::deserialize_u32(header_buf, 16, magic, offset)) return {};
    if (!IpcSerializer::deserialize_u32(header_buf, 16, length, offset)) return {};
    if (!IpcSerializer::deserialize_u32(header_buf, 16, status, offset)) return {};
    if (!IpcSerializer::deserialize_u32(header_buf, 16, sequence, offset)) return {};
...
#ifdef _WIN32
            int received = recv(sock_, reinterpret_cast<char*>(payload.data() + payload_received),
                               static_cast<int>(length - payload_received), 0);
#else
            ssize_t received = recv(sock_, payload.data() + payload_received, length - payload_received, 0);
#endif
            if (received <= 0) {
