...
protected:
     ...
     */
    bool parse_header(const std::vector<uint8_t>& response,
                      uint32_t& magic, uint32_t& length,
                      uint32_t& status_code, uint32_t& sequence_id,
                      size_t& offset) const {
        offset = 0;
        if (!IpcSerializer::deserialize_u32(response.data(), response.size(), magic, offset)) return false;
        if (!IpcSerializer::deserialize_u32(response.data(), response.size(), length, offset)) return false;
        if (!IpcSerializer::deserialize_u32(response.data(), response.size(), status_code, offset)) return false;
        if (!IpcSerializer::deserialize_u32(response.data(), response.size(), sequence_id, offset)) return false;
        return true;
    }
...
private:
     ...
     */
    std::unique_ptr<CommandHandler> get_command_handler(const std::string& command) {
        static const std::map<std::string, std::function<std::unique_ptr<CommandHandler>()>> handlers = {
    ...
    void print_usage() const {
        std::cout << "SMS Relay CLI - Command-line interface for sms_relay service\n\n";
        std::cout << "Usage:\n";
        ...
        std::cout << "                      Can be hostname, IPv4, or IPv6 address\n";
        ...
        std::cout << "  sms_relay_cli send --to \"+1234567890\" --text \"Hello World\"\n";
