...
[[nodiscard]] inline std::string gsm7_get_char(uint8_t position)
{
    ...
    switch (position)
    {
        ...
        case 0:
            return "\x40"; // @
        case 1:
            return "\xC2\xA3"; // £ (UTF-8)
        case 2:
            return "\x24"; // $
        case 3:
            return "\xC2\xA5"; // ¥ (UTF-8)
        case 4:
            return "\xC3\xA8"; // è (UTF-8)
        case 5:
            return "\xC3\xA9"; // é (UTF-8)
        case 6:
            return "\xC3\xB9"; // ù (UTF-8)
        case 7:
            return "\xC3\xAC"; // ì (UTF-8)
        case 8:
            return "\xC3\xB2"; // ò (UTF-8)
        case 9:
            return "\xC3\x87"; // Ç (UTF-8)
        case 10:
            return "\x0A"; // \n
        case 11:
            return "\xC3\x98"; // Ø (UTF-8)
        case 12:
            return "\xC3\xB8"; // ø (UTF-8)
        case 13:
            return "\x0D"; // \r
        case 14:
            return "\xC3\x85"; // Å (UTF-8)
        case 15:
            return "\xC3\xA5"; // å (UTF-8)
        // Positions 16-27: Greek letters
        case 16:
            return "\xCE\x94"; // Δ (UTF-8)
        case 17:
            return "\x5F"; // _
        case 18:
            return "\xCE\xA6"; // Φ (UTF-8)
        case 19:
            return "\xCE\x93"; // Γ (UTF-8)
        case 20:
            return "\xCE\x9B"; // Λ (UTF-8)
        case 21:
            return "\xCE\xA9"; // Ω (UTF-8)
        case 22:
            return "\xCE\xA0"; // Π (UTF-8)
        case 23:
            return "\xCE\xA8"; // Ψ (UTF-8)
        case 24:
            return "\xCE\xA3"; // Σ (UTF-8)
        case 25:
            return "\xCE\x98"; // Θ (UTF-8)
        case 26:
            return "\xCE\x9E"; // Ξ (UTF-8)
        case 27:
            return "\x1B"; // ESC
        // Positions 28-31
        case 28:
            return "\xC3\x86"; // Æ (UTF-8)
        case 29:
            return "\xC3\xAE"; // æ (UTF-8)
        case 30:
            return "\xC3\x9F"; // ß (UTF-8)
        case 31:
            return "\xC3\x89"; // É (UTF-8)
...
[[nodiscard]] inline uint32_t utf8_to_codepoint(std::string_view str,
                                                size_t &pos)
{
    ...
    {
        code_point = ((c & 0x0F) << 12) | ((str[pos + 1] & 0x3F) << 6) |
                     (str[pos + 2] & 0x3F);
        char_len = 3;
    }
...
[[nodiscard]] inline bool gsm7_find_position(uint32_t code_point,
                                             uint8_t &position)
{
...
gsm7_encode(std::string_view text, bool discard_invalid = false)
{
    std::vector<uint8_t> result;
    ...
    for (size_t i = 0; i < text.length();)
    {
        ...
        else
        {
            ...
            {
                if (static_cast<uint32_t>(static_cast<unsigned char>(ext.first)) ==
                    code_point)
                {
            ...
            if (!found && !discard_invalid)
            {
                throw std::invalid_argument(
                    "Cannot encode character in GSM-7: code point 0x" +
...
[[nodiscard]] inline std::string
gsm7_decode(const std::vector<uint8_t> &encoded)
{
    std::string result;
    ...
    for (size_t i = 0; i < encoded.size(); ++i)
    {
        ...
        if (byte_val == 0x1B && i + 1 < encoded.size())
        { // Escape
...
[[nodiscard]] inline std::vector<uint8_t>
septets_pack(const std::vector<uint8_t> &septets, size_t pad_bits = 0)
{
    if (septets.empty())
        return {};
...
[[nodiscard]] inline std::vector<uint8_t>
septets_unpack(const std::vector<uint8_t> &packed, size_t num_septets = 0,
               uint8_t prev_octet = 0, int shift = 7)
{
    std::vector<uint8_t> result;

    if (num_septets == 0)
        num_septets = packed.size() * 8 / 7 + 1;
...
[[nodiscard]] inline std::string
ucs2_decode(const std::vector<uint8_t> &encoded)
{
    std::string result;
...
[[nodiscard]] inline std::vector<uint8_t>
semi_octets_encode(std::string_view number)
{
...
[[nodiscard]] inline std::string
semi_octets_decode(const std::vector<uint8_t> &encoded, size_t num_octets = 0)
{
    std::string result;
...
[[nodiscard]] inline AddressField build_address_field(std::string_view address,
                                                      bool smsc_field = false)
{
    AddressField result;
    ...
    else
    {
        ...
        result.length = smsc_field ? static_cast<uint8_t>(result.encoded.size() + 1)
...
[[nodiscard]] inline AddressField
parse_address_field(std::vector<uint8_t>::const_iterator &it,
                    bool smsc_field = false)
{
    AddressField result;
    result.length = *it++;
    ...
    // Numeric address
    size_t octet_count =
        smsc_field ? (result.length - 1) : ((result.length + 1) / 2);
    std::string decoded = semi_octets_decode(
        std::vector<uint8_t>(it, it + octet_count), octet_count);
    it += octet_count;
...
[[nodiscard]] inline std::string
parse_timestamp(std::vector<uint8_t>::const_iterator &it)
{
    ...
    if (decoded.length() >= 14)
    {
        ...
        std::string formatted = ts.substr(0, 2) + "/" + ts.substr(2, 2) + "/" +
                                ts.substr(4, 2) + " " + ts.substr(6, 2) + ":" +
...
[[nodiscard]] inline UdhInfo parse_udh(std::vector<uint8_t>::const_iterator &it,
                                       uint8_t udh_len)
{
    UdhInfo info;
    info.present = true;
...
[[nodiscard]] inline std::vector<uint8_t> encode_submit_pdu_internal(
    std::string_view number, std::string_view text, uint8_t reference,
    std::string_view smsc, uint8_t dcs, bool with_udh = false,
    uint16_t concat_ref = 0, uint8_t concat_seq = 0, uint8_t concat_total = 0)
{
...
std::vector<std::string> encode_submit(const std::string &number,
                                       ...
                                       const std::string &smsc)
{
    ...
    {
        size_t with_udh_max =
            (dcs == DCS_GSM7) ? MAX_GSM7_WITH_UDH : MAX_UCS2_WITH_UDH;
        ...
        for (size_t part = 0; part < total_parts; ++part)
        {
            ...
            auto pdu_bytes = detail::encode_submit_pdu_internal(
                number, part_text, reference, smsc, dcs, true, reference,
                static_cast<uint8_t>(part + 1), static_cast<uint8_t>(total_parts));
    ...
    // Single PDU
    auto pdu_bytes =
        detail::encode_submit_pdu_internal(number, text, reference, smsc, dcs);
...
PduMessage decode(const std::vector<uint8_t> &pdu_bytes)
{
    ...
    if (pdu_type == constants::MT_SMS_DELIVER)
    {
        ...
        if (udh_present)
        {
            ...
            // Create remaining from UDH length byte position (include UDH in septet
            // unpacking)
    ...
    {
        throw std::invalid_argument("Unknown PDU type: " +
                                    std::to_string(pdu_type));
    }
