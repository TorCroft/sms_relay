...
struct RequestHeader
{
    ...
    RequestHeader(uint32_t cmd, uint32_t seq = 0)
        : magic(IPC_MAGIC), length(0), command_type(static_cast<uint32_t>(cmd)),
          sequence_id(seq) {}
};
...
struct ResponseHeader
{
    ...
    ResponseHeader(uint32_t stat, uint32_t seq)
        : magic(IPC_MAGIC), length(0), status(static_cast<uint32_t>(stat)),
          sequence_id(seq) {}
};
...
struct SmsInfo
{
    ...
    uint8_t status; // Message status: 0=REC UNREAD, 1=REC READ, 2=STO UNSENT,
                    // 3=STO SENT
