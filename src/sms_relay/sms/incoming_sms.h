...
struct IncomingSms {
    ...
    bool is_last_part() const { return is_multipart() && concat_seq == concat_total; }

    // Get a unique identifier for this multipart conversation
    std::string get_conversation_id() const {
        if (!is_multipart()) return std::to_string(index);
...
struct MultipartSmsCache {
    ...
    bool has_part(uint8_t seq) const {
        for (const auto& p : parts) {
            if (p.concat_seq == seq) return true;
        }
        return false;
    }
    ...
    IncomingSms combine() const {
        if (parts.empty()) return IncomingSms{};
