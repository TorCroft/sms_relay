...
namespace smsrelay
{
    ...
    SmsService::SmsService(std::shared_ptr<at::AtSession> at_session, const SmsConfig &config)
    ...
    void SmsService::handle_cmti(const std::string & /* urc */, const std::string &args)
    {
    ...
    IncomingSms SmsService::read_sms(const std::string &storage, uint8_t index)
    {
        ...
        try
        {
            ...
            if (!response.success)
            {
                std::cerr << "[SMS] Failed to read message (status: " << response.status << ")" << std::endl;
                return sms;
            }
    ...
    std::pair<std::string, uint8_t> SmsService::parse_cmti_args(const std::string &args)
    {
        // Format: "ME",8 or "SM",5
        std::regex regex(R"(\"([^\"]+)\",(\d+))");
        std::smatch match;
    ...
    std::string SmsService::extract_pdu_from_cmgr(const at::ResponseBuilder::AtResponse &response)
    {
        ...
        {
            // Check if line looks like hex PDU (should be fairly long and only hex chars)
            if (line.length() > 20)
            {
                bool is_hex = true;
                for (char c : line)
    ...
    std::string SmsService::get_cache_key(const std::string &sender, uint16_t concat_ref) const
    {
    ...
    void SmsService::cleanup_cache()
    {
        ...
        for (auto it = multipart_cache_.begin(); it != multipart_cache_.end();)
        {
            ...
            {
                std::cerr << "[SMS] Multipart timeout for " << it->first
                          << ", got " << it->second.parts.size()
                          << "/" << it->second.total_parts << " parts" << std::endl;
    ...
    std::vector<IncomingSms> SmsService::list_and_decode_messages()
    {
        ...
        std::lock_guard<std::mutex> lock(cache_mutex_messages_);
        result = message_cache_; // Copy all (already decoded and combined)

        return result;
    }

    std::string SmsService::delete_message(const std::string &storage, uint8_t index)
    {
        try
        {
            ...
            for (uint8_t idx : indices_to_delete)
            {
                ...
                if (!response.success)
                {
                    std::string error = "Failed to delete index " + std::to_string(static_cast<int>(idx));
                ...
                std::this_thread::sleep_for(std::chrono::milliseconds(config_.read_delay_ms));
            }

            return last_error.empty() ? "" : last_error; // Return last error, or empty if all succeeded
        }
        catch (const std::exception &e)
        {
    ...
    void SmsService::initialize_cache()
    {
        refresh_cache();
    }
    ...
    void SmsService::refresh_cache()
    {
        try
        {
            // Read all messages without combining
            std::vector<IncomingSms> all_messages;

            // Get list of all message indices
            ...
            for (const auto &line : response.data)
            {
                if (line.find("+CMGL:") == 0)
                {
                    ...
                    {
                        std::string index_str = line.substr(colon_pos + 1, first_comma - colon_pos - 1);
                        std::string status_str = line.substr(first_comma + 1, second_comma - first_comma - 1);
                        try
                        {
                        ...
                        {
                            std::cerr << "[SMS] Failed to parse index/status: " << e.what() << std::endl;
                        }
                    }
                }
            }

            // Read and decode each message
            ...
            // Combine multipart messages before caching
            ...
            for (auto &sms : all_messages)
            {
                if (sms.decoded.has_udh && sms.decoded.concat_total > 1)
                {
                    // Multipart - collect parts
                    std::string cache_key = get_cache_key(sms.decoded.number, sms.decoded.concat_ref);
            ...
            for (auto &[key, entry] : multipart_map)
            {
                if (entry.is_complete())
                {
                    ...
                    IncomingSms combined;
                    combined.index = entry.first_index; // Use minimum (first) index
                    combined.storage = "ME";
                    ...
                    for (const auto &sms : all_messages)
                    {
                ...
                else
                {
                    ...
                    for (auto &sms : all_messages)
                    {
                        std::string sms_key = get_cache_key(sms.decoded.number, sms.decoded.concat_ref);
                        if (sms_key == key)
                        {
                            combined_messages.push_back(sms);
                        }
    ...
    void SmsService::remove_from_cache(uint8_t index)
    {
        ...
        auto it = std::remove_if(message_cache_.begin(), message_cache_.end(),
                                 [index](const IncomingSms &sms)
                                 {
                                     return sms.index == index;
                                 });
    ...
    void SmsService::add_to_cache(const IncomingSms &sms)
    {
        ...
        auto it = std::find_if(message_cache_.begin(), message_cache_.end(),
                               [&sms](const IncomingSms &cached)
                               {
                                   return cached.index == sms.index;
                               });
