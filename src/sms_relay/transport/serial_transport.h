...
struct SerialConfig {
    ...
    SerialConfig(const std::string& p, uint32_t b)
        : port(p), baudrate(b) {}
};
...
public:
    ...
    void set_data_callback(DataCallback cb) override { data_callback_ = std::move(cb); }
    void set_error_callback(ErrorCallback cb) override { error_callback_ = std::move(cb); }
