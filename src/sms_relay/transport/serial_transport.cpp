...
SerialTransport::SerialTransport(asio::io_context& io_ctx, const SerialConfig& config)
    : io_ctx_(io_ctx)
    , port_(io_ctx)
    , config_(config)
    , connected_(false)
...
void SerialTransport::do_open() {
    try {
        ...
        connected_ = false;
        std::string error_msg = "Failed to open serial port " + config_.port + ": " + e.what();
        if (error_callback_) {
...
void SerialTransport::handle_write(const asio::error_code& ec, size_t /* bytes_transferred */) {
    if (ec) {
        if (error_callback_) {
...
void SerialTransport::handle_read(const asio::error_code& ec, size_t bytes_transferred) {
    if (ec) {
