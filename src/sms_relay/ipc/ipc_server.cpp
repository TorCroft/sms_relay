...
namespace smsrelay::ipc
{
...
#ifdef _WIN32
            SOCKET client_fd = accept(server_fd_, reinterpret_cast<struct sockaddr *>(&client_addr), &addr_len);
    ...
    void IpcServer::handle_client(int client_fd)
    {
        ...
        {
#ifdef _WIN32
            int received = recv(client_fd, reinterpret_cast<char *>(header_buf + header_received),
    ...
    std::vector<uint8_t> IpcServer::handle_list(const std::vector<uint8_t> &payload)
    {
        // Parse payload
        size_t offset = 0;
        std::string status;
    ...
    std::vector<uint8_t> IpcServer::handle_read(const std::vector<uint8_t> &payload)
    {
        // Parse payload
        size_t offset = 0;
        ...
        // Read messages directly from cache (cache contains already combined messages)
        std::vector<uint8_t> response_payload;
        ...
        // Serialize results
        auto count_bytes = IpcSerializer::serialize_u32(static_cast<uint32_t>(results.size()));
        response_payload.insert(response_payload.end(), count_bytes.begin(), count_bytes.end());
    ...
    std::vector<uint8_t> IpcServer::handle_delete(const std::vector<uint8_t> &payload)
    {
        // Parse payload
        size_t offset = 0;
        std::vector<uint8_t> indices;
        ...
        for (uint8_t index : indices)
        {
            std::string error = sms_service_->delete_message(sms_service_->get_storage(), index);
            if (error.empty())
            {
            ...
            else
            {
                ...
                std::cerr << "[IPC Server] Failed to delete index " << static_cast<int>(index)
                         << ": " << error << std::endl;
            }
        }
        ...
        response_payload.insert(response_payload.end(), deleted_bytes.begin(), deleted_bytes.end());
        response_payload.insert(response_payload.end(), failed_bytes.begin(), failed_bytes.end());

        return IpcSerializer::serialize_response(Status::SUCCESS, 0, response_payload);
    }

    std::vector<uint8_t> IpcServer::handle_send(const std::vector<uint8_t> &payload)
    {
        // Parse payload
        size_t offset = 0;
        ...
        if (!IpcSerializer::deserialize_str(payload.data(), payload.size(), recipient, offset))
        {
            return IpcSerializer::serialize_response(Status::INVALID_ARGS, 0, {});
        }
        if (!IpcSerializer::deserialize_str(payload.data(), payload.size(), text, offset))
        {
            return IpcSerializer::serialize_response(Status::INVALID_ARGS, 0, {});
        }
        ...
        else
        {
            ...
            response_payload.insert(response_payload.end(), error_bytes.begin(), error_bytes.end());
            return IpcSerializer::serialize_response(Status::FAILED, 0, response_payload);
        }
    }

    std::vector<uint8_t> IpcServer::handle_status(const std::vector<uint8_t> &payload)
    {
