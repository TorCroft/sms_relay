...
AtSession::AtSession(std::shared_ptr<transport::SerialTransport> transport,
    ...
    : io_ctx_(io_ctx), transport_(transport), response_builder_(),
      dispatcher_(response_builder_, urc_dispatcher_)
{
    // Set up line parser callback
    line_parser_.set_line_callback(
        [this](const std::string &line) { on_line_parser_line(line); });

    // Set up transport callbacks
    transport_->set_data_callback([this](const std::vector<uint8_t> &data) {
        on_transport_data_received(data);
    });

    transport_->set_error_callback(
        [this](const std::string &error) { on_transport_error(error); });

    // Register common URC handlers
    urc_dispatcher_.register_handler(
        "+CMTI", [this](const std::string &urc, const std::string &args) {
            on_cmti_urc(urc, args);
        });
}

AtSession::~AtSession() { stop(); }
...
std::future<AtSession::AtResponse>
AtSession::send_command(std::string cmd, std::chrono::milliseconds timeout)
{
...
void AtSession::process_next_command()
{
    ...
    current_command_ = std::make_unique<CurrentCommand>(
        CurrentCommand{queued_cmd.text, queued_cmd.promise, pending_timeout_,
    ...
    current_command_->timer.async_wait(
        [this](const asio::error_code &ec) { on_timeout(ec); });
...
void AtSession::on_line_received(const std::string &line)
{
    ...
    else if (response_builder_.state() ==
             ResponseBuilder::State::WaitingPrompt)
    {
...
std::future<AtSession::AtResponse>
AtSession::set_urc_port(const std::string &port)
{
...
std::future<AtSession::AtResponse> AtSession::check_and_set_urc_port()
{
    ...
    // Use get_urc_port() first, check the response, then call set_urc_port() if
    // needed
...
{
    return send_command(pdu_mode ? commands::SET_SMS_PDU_MODE
                                 : commands::SET_SMS_TEXT_MODE);
}
...
std::future<AtSession::AtResponse>
AtSession::list_messages(const std::string &stat)
{
...
std::future<AtSession::AtResponse>
AtSession::send_message(const std::string &pdu)
{
    ...
    auto future =
        send_command(commands::send_message(static_cast<uint16_t>(pdu_length)));
...
// ============================================================================
// Callback Handlers (Extracted from lambdas)
// ============================================================================

void AtSession::on_line_parser_line(const std::string &line)
{
    // Post to IO thread for processing
    asio::post(io_ctx_, [this, line]() { on_line_received(line); });
}

void AtSession::on_transport_data_received(const std::vector<uint8_t> &data)
{
    process_data_from_transport(data);
}

void AtSession::on_transport_error(const std::string &error)
{
    ...
    if (command_in_progress_ && current_command_ && current_command_->promise)
    {
        ...
        asio::post(io_ctx_, [this]() { process_next_command(); });
    }
}

void AtSession::on_cmti_urc(const std::string &urc, const std::string &args)
{
    if (urc_callback_)
    {
